...
class Player {
  ...
  constructor(playerData) {
    ...
    this.positionabrev = playerData.position_abbreviation;
    this.jerseyNumber = playerData.jersey_number;
    this.college = playerData.college;
    this.team= playerData.team.name;
...
function NFLPlayerSearch() {
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(
        `/nflplayer?search=${encodeURIComponent(search)}`
      );
      if (!response.ok) {
  ...
  return (
    <div className="nfl-ps-header">
      <h1 className="text-xl font-bold">NFL Player Search</h1>
        ...
        {players.map((p) => (
          <div key={p.id} className={`player-card nfl-${p.teamabbreviation} border p-2 my-2`}>
            ...
            <div className="player-attributes">
              ...
              <div>
                Position: {p.position} ({p.position_abbreviation})
              </div>
              <div>
